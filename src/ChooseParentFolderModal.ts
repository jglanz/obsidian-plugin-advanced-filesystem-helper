import { App, FuzzySuggestModal, TFolder, Vault, MarkdownView, TFile, Platform, normalizePath } from 'obsidian';
...
export default class ChooseParentFolderModal extends FuzzySuggestModal<TFolder> {
  folders: TFolder[];
  ...
  onChooseItem(item: TFolder, evt: MouseEvent | KeyboardEvent): void {
    if (this.newDirectoryPath?.length > 0) {
      this.createDirectory(item, this.newDirectoryPath)
